...
def eval(im1_np,im2_np):
    #im1_np = numpy.array(PIL.Image.open(im1))
    #im2_np = numpy.array(PIL.Image.open(im2))                    
    tenOne = torch.FloatTensor(numpy.ascontiguousarray(im1_np)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) )
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(im2_np)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) )
    
    res = estimate(tenOne, tenTwo)
    return res
# end
